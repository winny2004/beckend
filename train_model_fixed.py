...
def train_random_forest(data_path, model_output_path='models/random_forest_model.pkl', 
    ...
    if data_path.endswith('.xlsx'):
        ...
        # Convert DASS21 columns to numeric (scale 0-3)
        for i in range(1, 22):
            col = f'DAS{i}'
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(int)
                # Ensure values are within valid range (0-3) for DASS-21 scale
                df[col] = df[col].clip(0, 3)
                
        print(f"OK DASS-21 scale validated (0-3)")
    else:
    ...
    # Validate DASS-21 scale range
    dass_cols = [col for col in df.columns if col.startswith('DAS')]
    for col in dass_cols:
        min_val = df[col].min()
        max_val = df[col].max()
        if min_val < 0 or max_val > 3:
            print(f"Warning: {col} has values outside 0-3 range (min: {min_val}, max: {max_val})")
            df[col] = df[col].clip(0, 3)
    
    print("OK All DASS-21 columns validated (0-3 scale)")
    ...
    # Hitung skor DASS (each item is 0-3, sum and multiply by 2)
    ...
    # Show score ranges
    print("\nScore Ranges:")
    print(f"Depression: {df['Depression_Score'].min()} - {df['Depression_Score'].max()}")
    print(f"Anxiety: {df['Anxiety_Score'].min()} - {df['Anxiety_Score'].max()}")
    print(f"Stress: {df['Stress_Score'].min()} - {df['Stress_Score'].max()}")
